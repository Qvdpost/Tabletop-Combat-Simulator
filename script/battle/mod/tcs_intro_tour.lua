...
function st_helper.get_building_from_list(list, filter)
	...
	if not is_function(filter) then
		script_error("ERROR: get_building_from_list() called but supplied filter [" ..
		tostring(filter) .. "] is not a function");
		return false;
	end;
...
function st_helper.get_closest_building_from_list(pos, list, filter)
	if not is_vector(pos) then
		script_error("ERROR: get_closest_building_from_list() called but supplied position [" ..
		tostring(pos) .. "] is not a vector");
		return false;
	end;

	if not is_table(list) then
		script_error("ERROR: get_closest_building_from_list() called but supplied list [" ..
		tostring(list) .. "] is not a table");
		return false;
	end;
	...
	if filter and not is_function(filter) then
		script_error("ERROR: get_closest_building_from_list() called but supplied filter [" ..
		tostring(filter) .. "] is not a function or nil");
		return false;
	end;
	...
	if not closest_building then
		script_error(
		"WARNING: get_closest_building_from_list() couldn't find an eligible building, returning first in list");
...
function st_helper.get_closest_connected_wall_building(pos, connected_n_times, include_nonstandard_connections)
	if not is_vector(pos) then
		script_error("ERROR: get_closest_connected_wall_building() called but supplied position [" ..
		tostring(pos) .. "] is not a vector");
		return false;
	end;
	...
	script_error(
	"ERROR: get_closest_connected_wall_building() couldn't find a fort wall building, are we not in a siege?");
end;
...
function st_helper.capture_location_of_type_exists(capture_location_type, match_partial)
	if not is_string(capture_location_type) then
		script_error("ERROR: capture_location_of_type_exists() called but supplied capture location type [" ..
		tostring(capture_location_type) .. "] is not a string");
		return false;
	end;
	...
	if match_partial then
		for i = 0, num_capture_points - 1 do
			local current_capture_location_type = common.get_context_value("CcoBattleRoot", "",
				"CapturePointList().At(" .. i .. ").TypeRecordContext().Key");
...
function st_helper.capture_location_of_type_with_toggleable_slot_exists(capture_location_type, match_partial)
	...
	for i = 0, common.get_context_value("CcoBattleRoot", "", "CapturePointList().Size") - 1 do
		...
		if not capture_location_type_matches then
			local current_capture_location_type = common.get_context_value("CcoBattleRoot", "",
				capture_location_type_record .. ".Key");
...
function st_helper.get_closest_capture_location_of_type(pos, capture_location_type, match_partial)
	if not is_vector(pos) then
		script_error("ERROR: get_closest_capture_location_of_type() called but supplied position [" ..
		tostring(pos) .. "] is not a vector");
		return false;
	end;

	if capture_location_type and not is_string(capture_location_type) then
		script_error("ERROR: get_closest_capture_location_of_type() called but supplied capture location type [" ..
		tostring(capture_location_type) .. "] is not a string");
		return false;
	end;
	...
	for i = 0, common.get_context_value("CcoBattleRoot", "", "CapturePointList().Size") - 1 do
		...
		if not type_match then
			if match_partial then
				type_match = string.find(
				common.get_context_value("CcoBattleRoot", "", capture_location_type_record .. ".Key"),
					capture_location_type);
			else
				type_match = common.get_context_value("CcoBattleRoot", "", capture_location_type_record .. ".Key") ==
				capture_location_type;
			end;
		end;

		if type_match then
			local current_distance = v_from_context_str("CcoBattleRoot", "", capture_location_str .. ".FlagPosition")
			:distance(pos);
			if current_distance < closest_distance then
				closest_distance = current_distance;
...
function st_helper.get_closest_toggleable_slot_from_capture_location_of_type(pos, capture_location_type, match_partial)
	if not is_vector(pos) then
		script_error("ERROR: get_closest_toggleable_slot_from_capture_location_of_type() called but supplied position [" ..
		tostring(pos) .. "] is not a vector");
		return false;
	end;
	...
	if capture_location_type and not is_string(capture_location_type) then
		script_error(
		"ERROR: get_closest_toggleable_slot_from_capture_location_of_type() called but supplied capture location type [" ..
		tostring(capture_location_type) .. "] is not a string");
		return false;
	end;
	...
	for i = 0, common.get_context_value("CcoBattleRoot", "", "CapturePointList().Size") - 1 do
		...
		if type_match then
			...
			for j = 0, common.get_context_value("CcoBattleRoot", "", toggleableslotlist_str .. ".Size") - 1 do
				...
				local current_distance = v_from_context_str("CcoBattleRoot", "", current_slot_str .. ".WorldPosition")
				:distance(pos);
				if current_distance < closest_distance then
					closest_distance = current_distance;
...
function st_helper.get_second_offset_camera_position(cam_target, initial_position, horizontal_bearing_delta)
	...
	if horizontal_bearing_delta == 0 then
		script_error(
		"WARNING: get_second_offset_camera_position() called with a horizontal bearing delta of 0 - the returned camera position will be the same as the supplied camera position");
...
function st_helper.get_offset_camera_positions_by_bearing(cam_target, distance, horizontal_bearing, vertical_bearing,
														  horizontal_bearing_delta)
	if not validate.is_vector(cam_target) then
		return false;
	end;
...
--- @r @battle_vector start camera position
--- @r @battle_vector end camera position (or @nil if delta is 0)
function st_helper.get_offset_camera_positions_by_offset_and_bearing(cam_target, x_offset, z_offset, horizontal_bearing,
																	 vertical_bearing, horizontal_bearing_delta)
	...
	return first_camera_pos,
		st_helper.get_second_offset_camera_position(cam_target, first_camera_pos, horizontal_bearing_delta);
end;
...
--- @r @battle_vector start camera position
--- @r @battle_vector end camera position (or @nil if delta is 0)
function st_helper.get_offset_camera_positions_by_bearing_vector(cam_target, bearing_pos, distance, vertical_bearing,
																 horizontal_bearing_delta)
	return st_helper.get_offset_camera_positions_by_bearing(cam_target, distance, get_bearing(cam_target, bearing_pos),
		vertical_bearing, horizontal_bearing_delta);
end;
...
--- @r @battle_vector start camera position
--- @r @battle_vector end camera position (or @nil if delta is 0)
function st_helper.get_offset_camera_positions_from_sunit(sunit, x_offset, z_offset, vertical_bearing,
														  horizontal_bearing_delta)
	...
	return unit_position,
		st_helper.get_offset_camera_positions_by_offset_and_bearing(unit_position, x_offset, z_offset,
			sunit.unit:bearing(), vertical_bearing, horizontal_bearing_delta);
end;
...
--- @r @battle_vector start camera position
--- @r @battle_vector end camera position (or @nil if delta is 0)
function st_helper.get_offset_camera_positions_from_sunits(sunits, x_offset, z_offset, horizontal_bearing,
														   vertical_bearing, horizontal_bearing_delta,
														   relaxed_camera_pose)
	...
	return centre_pos,
		st_helper.get_offset_camera_positions_by_offset_and_bearing(centre_pos, x_offset, z_offset, horizontal_bearing,
			vertical_bearing, horizontal_bearing_delta);
end;
...
function st_helper.get_offset_camera_positions_from_walls_as_defender()
	...
	if sunits_player then
		sunits_on_walls = sunits_player:filter("sunits_on_walls",
			function(sunit) return sunit.unit:is_on_top_of_platform() end);
		...
		for i = 1, sunits_on_walls:count() do
			...
			local closest_gate_building = st_helper.get_closest_building_from_list(current_sunit_position,
				bm:get_fort_gate_buildings());
...
function st_helper.get_offset_camera_positions_from_gate_capture_location()
	...
	for i = 0, common.get_context_value("CcoBattleRoot", "", "CapturePointList().Size") - 1 do
		...
		if common.get_context_value("CcoBattleRoot", "", capture_location_str .. ".HasCaptureBuilding") then
			...
			if common.get_context_value("CcoBattleRoot", "", building_str .. ".IsGate") then
				local pos_capture_location = v_from_context_str("CcoBattleRoot", "",
					capture_location_str .. ".FlagPosition");
	...
	if #eligible_capture_location_records == 0 then
		script_error(
		"ERROR: get_offset_camera_positions_from_gate_capture_location() could find no gate capture locations");
		return false;
	end;
	...
	local pos_offset_from_gate = position_along_line(pos_gate, pos_capture_location,
		pos_gate:distance(pos_capture_location) * 2);
...
function st_helper.get_offset_camera_positions_from_siege_weapons()
	...
	if closest_vehicle then
		...
		script_error(
		"WARNING: get_offset_camera_positions_from_siege_weapons() could find no siege weapons - will compute camera positions from attacking general instead");
...
function st_helper.get_offset_camera_positions_from_minor_point_supplies_toggleable_slot_location()
	local closest_slot_location_str = st_helper.get_closest_toggleable_slot_from_capture_location_of_type(cam:position(),
		"minor_point_supplies", true);

	if not closest_slot_location_str then
		script_error(
		"ERROR: get_offset_camera_positions_from_minor_point_supplies_toggleable_slot_location() cannot find any minor key building capture locations");
		return false;
	end;
...
function st_helper.get_offset_camera_positions_from_minor_point_supplies_capture_location()
	local closest_location_str = st_helper.get_closest_capture_location_of_type(cam:position(), "minor_point_supplies",
		true);

	if not closest_location_str then
		script_error(
		"ERROR: get_offset_camera_positions_from_minor_point_supplies_capture_location() cannot find any minor key building capture locations");
		return false;
	end;
...
function st_helper.get_offset_camera_positions_from_major_victory_point()
	...
	if not closest_location_str then
		script_error(
		"ERROR: get_offset_camera_positions_from_victory_point() cannot find any minor key building capture locations");
		return false;
	end;
...
function st_helper.get_offset_camera_positions_from_minor_victory_point()
	local closest_location_str = st_helper.get_closest_capture_location_of_type(cam:position(), "major_key_building",
		true);
...
function st_helper.navigable_tour_section_battle_factory(section_name, camera_positions_generator, advice_key, infotext,
														 advice_infotext_delay, objective_key, objective_completion_test,
														 leave_objective_after_completion, windowed_movie_path)
	if not is_string(section_name) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied section name [" ..
		tostring(section_name) .. "] is not a string");
		return false;
	end;

	if camera_positions_generator and not is_function(camera_positions_generator) then
		script_error(
		"ERROR: navigable_tour_section_battle_factory() called but supplied camera positions generator function [" ..
		tostring(camera_positions_generator) .. "] is not a function or nil");
		return false;
	end;

	if advice_key and not is_string(advice_key) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied advice key [" ..
		tostring(advice_key) .. "] is not a string");
		return false;
	end;

	if infotext and not is_table(infotext) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied infotext table [" ..
		tostring(infotext) .. "] is not a table or nil");
		return false;
	end;
	...
	if advice_infotext_delay then
		if not is_non_negative_number(advice_infotext_delay) and not is_string(advice_infotext_delay) then
			script_error(
			"ERROR: navigable_tour_section_battle_factory() called but supplied objective completion test [" ..
			tostring(objective_completion_test) .. "] is not a function");
			return false;
		end;
	else
		if advice_key then
	...
	if objective_key and not is_string(objective_key) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied objective key [" ..
		tostring(objective_key) .. "] is not a string");
		return false;
	end;

	if objective_completion_test and not is_function(objective_completion_test) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied objective completion test [" ..
		tostring(objective_completion_test) .. "] is not a function");
		return false;
	end;

	if windowed_movie_path and not is_string(windowed_movie_path) then
		script_error("ERROR: navigable_tour_section_battle_factory() called but supplied movie key [" ..
		tostring(windowed_movie_path) .. "] is not a string");
		return false;
	end;
											...
											bm:progress_on_advice_finished(
												...
												function()
													if nts:is_playing() then
														core:svr_save_registry_bool(
														"navigable_tour_progress_button_highlighted", true);
														nts:highlight_next_button();
													end;
												end,
												nil,
...
do
	tcs_introduction = navigable_tour:new(
		"deployment", -- unique name
		function() -- end callback
		end,
		"ui_text_replacements_localised_text_tcs_introduction_tour_name"
	);

	tcs_introduction:set_allow_camera_movement(true);

	-- validation rules for this navigable tour
	tcs_introduction:add_validation_rule(
		function()
	...
	tcs_introduction:add_validation_context_change_listener(
		"BattleConflictPhaseCommenced"
	);

	-------------------------------------------------
	--- Section 01 : Phase Controls
	-------------------------------------------------

	local tcs_control_panel = st_helper.navigable_tour_section_battle_factory(
		"phase_controls", -- section name
		function()
		...
		nil, -- advice key
		{
			"tcs.battle.advice.controls.info_001",
			"tcs.battle.advice.controls.info_002",
			"tcs.battle.advice.controls.info_003",
			"tcs.battle.advice.controls.info_004"
		} -- infotext
	);

	-- make the deployment zone flash, and then stop it flashing when we proceed
	tcs_control_panel:action(
		function()
			...
			tcs_control_panel:add_skip_action(
				function()
	...
	-------------------------------------------------
	--- Section 02 : Activations
	-------------------------------------------------

	local tcs_activations = st_helper.navigable_tour_section_battle_factory(
		"tcs_activations", -- section name
		function()
			return st_helper.get_offset_camera_positions_from_sunits(
				script_units:new("tour_general", general),
				0,
				-60,
				nil,
				nil,
		...
		nil, -- advice key
		{
			"tcs.battle.advice.activations.info_001",
			"tcs.battle.advice.activations.info_002",
			"tcs.battle.advice.activations.info_003",
			"tcs.battle.advice.activations.info_004"
		} -- infotext
	);

	tcs_activations:action(
		function()
	...
	tcs_activations:action(
		function()
			local move_button = get_ability_button_cco("tcs_main_unit_active_move")

			move_button:Highlight(true)

			tcs_activations:add_skip_action(
				function()
					move_button:Highlight(false)
					bm:steal_input_focus(false, true);
				end
			);
		end,
	...
	-------------------------------------------------
	--- Section 03 : Unit Info Positions
	-------------------------------------------------

	local tcs_unit_info = st_helper.navigable_tour_section_battle_factory(
		...
		nil, -- advice key
		{
			"tcs.battle.advice.unit_info.info_001",
			"tcs.battle.advice.unit_info.info_002",
			"tcs.battle.advice.unit_info.info_003",
			"tcs.battle.advice.unit_info.info_004"
		} -- infotext
	);

	tcs_unit_info:action(
		function()
			bm:steal_input_focus(true, true);
			...
			local unit_info_panel = find_uicomponent(root, "hud_battle", "info_panel_parent")

			unit_info_panel:SetVisible(true)

			local advice_panel = find_uicomponent(root, "under_advisor_docker", "info_text")

			advice_panel:TriggerAnimation("move_right")

			create_unit_status()

			local unit_status_component = create_unit_status()
			unit_status_component:Highlight(true, true)

			tcs_unit_info:add_skip_action(
				function()
					unit_info_panel:SetVisible(false)
	...
	tcs_introduction:add_navigable_section(tcs_control_panel);
	tcs_introduction:add_navigable_section(tcs_activations);
	tcs_introduction:add_navigable_section(tcs_unit_info);




	-- add startup actions
	tcs_introduction:start_action(
		function()
			st_helper.setup_tour_start(tcs_introduction);
		end,
		0
	);
	...
	-- add end actions
	tcs_introduction:end_action(
		function()
			bm:steal_input_focus(false, false);
			core:svr_save_registry_bool("tcs_introduction_tour", true)
			st_helper.setup_tour_end(tcs_introduction);
		end
	);
end;
